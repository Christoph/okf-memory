...
### 9.1 Optional extension contracts

A bundle MAY include an ordinary concept document that explains how local
extensions, skills, or agents should read and update that bundle. okf-memory
uses `EXTENSIONS.md` for this purpose, with `type: Reference` frontmatter.
Consumers should discover it through `index.md` like any other concept; they
must not require the file for OKF conformance.

A useful extension contract documents:

- progressive-disclosure reading from `index.md` to area indexes to concepts;
- concept IDs as bundle-relative paths without `.md`, and any local slug rules;
- required frontmatter and expectations to preserve unknown keys/types;
- safe-write behavior such as one concept per file, timestamp updates, index
  regeneration, and log entries; and
- local validation commands or fallback OKF conformance checks.
