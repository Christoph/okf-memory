 ...
 *
 * Shared client JS provides: __TOKEN (from the URL, echoed on every request),
 * __api(path), post(payload, okMsg), a pagehide cancel-beacon (server holds it
 * for a grace period so reloads don't cancel), cancelFlow() (explicit Cancel →
...
const SHARED_JS = `
var __TOKEN = new URLSearchParams(location.search).get('t') || '';
function __api(path) {
  return path + (path.indexOf('?') >= 0 ? '&' : '?') + 't=' + encodeURIComponent(__TOKEN);
}
var __done = false;
